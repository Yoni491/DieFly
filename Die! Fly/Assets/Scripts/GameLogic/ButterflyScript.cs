...
public class ButterflyScript : MonoBehaviour
{
    ...
    public void CreateButterfly(int i_MaxLife, float i_Speed, int i_FlyingMethod)
    {
    ...
    void FlyingAlgorithm()
    {
        ...
        if ((ButterflyManager.s_PlayerInteractionPoints[1].position.y - 0.3 > transform.position.y) && !isFlyingToPoint) //make it enum !@!@#!@#!@#!@#
        {
    ...
    void FlyToNewPoint()
    {
        ...
        {
            m_CurrentflyingPoint = ButterflyManager.s_FoodPoints[0].position;
            isFlyingToFood = true;
        }
        else//flying around
        {
            float tempX = Random.Range(ButterflyManager.s_PlayerInteractionPoints[0].position.x, ButterflyManager.s_PlayerInteractionPoints[1].position.x);
            float tempY = Random.Range(ButterflyManager.s_PlayerInteractionPoints[0].position.y, ButterflyManager.s_PlayerInteractionPoints[1].position.y);
            float tempZ = Random.Range(ButterflyManager.s_PlayerInteractionPoints[0].position.z, ButterflyManager.s_PlayerInteractionPoints[1].position.z);
